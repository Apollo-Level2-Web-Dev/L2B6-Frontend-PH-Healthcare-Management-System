            ...
            <div className={`flex flex-col gap-1.5 max-w-[78%] ${isUser ? "items-end" : "items-start"}`}>
                ...
                {!isUser && message.sources && message.sources.length > 0 && (
                    <div className="flex flex-wrap gap-1 px-1">
                        <span className="text-[10px] text-slate-400 w-full mb-0.5">Sources:</span>
...
export default function FloatingChatbot() {
        ...
        startSyncTransition(async () => {
            ...
            if (result.success) {
                toast.success(`Doctor data synced!`, {
                    description: result.message ?? `${result.indexedCount ?? 0} doctors indexed.`,
                });
            } else {
    ...
    return (
        <>
            {/* ── Chat Window ────────────────────────────────────────────────── */}
            <div
                className={`fixed bottom-24 right-6 z-50 w-[22rem] sm:w-[26rem] flex flex-col rounded-2xl shadow-2xl border border-slate-200/80 overflow-hidden bg-white transition-all duration-300 ease-in-out ${
                    isOpen
                ...
                <div className="bg-gradient-to-r from-blue-600 to-violet-600 px-4 py-3 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            ...
                            <p className="text-blue-100 text-[10px] mt-0.5">Powered by RAG · Always online</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-1">
                        {/* Sync button */}
                ...
                >
                    ...
                    {messages.length === 1 && !isQuerying && (
                        <div className="flex flex-col gap-2 mt-2">
                            <p className="text-[11px] text-slate-400 font-medium px-1">Try asking:</p>
